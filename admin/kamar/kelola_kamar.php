...
<div class="main-content">
    ...
    <?php while ($loc = mysqli_fetch_assoc($query_lokasi)) : $nama_lokasi = $loc['lokasi']; ?>
        <div class="location-header shadow-sm mb-3">
            <i class="fas fa-map-marker-alt text-danger me-2"></i> LOKASI: <strong><?= strtoupper(htmlspecialchars($nama_lokasi, ENT_QUOTES)); ?></strong>
        </div>
        ...
        <div class="card border-0 shadow-sm rounded-4 p-3 mb-4">
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        <?php
                        $query_kamar = mysqli_query($conn, "SELECT * FROM kamar WHERE lokasi = '".mysqli_real_escape_string($conn, $nama_lokasi)."' ORDER BY LENGTH(nama_kamar) ASC, nama_kamar ASC");
                        while ($row = mysqli_fetch_assoc($query_kamar)) :
                        ?>
                        <tr>
                            <td class="ps-3"><img src="../../img/<?= htmlspecialchars($row['gambar'], ENT_QUOTES); ?>" class="img-kamar shadow-sm" onerror="this.src='https://via.placeholder.com/150'" alt="<?= htmlspecialchars($row['nama_kamar'], ENT_QUOTES); ?>"></td>
                            <td>
                                <div class="fw-bold"><?= htmlspecialchars($row['nama_kamar'], ENT_QUOTES); ?></div>
                                <span class="badge bg-light text-dark border fw-normal"><i class="fas fa-<?= $row['tipe'] == 'ikhwan' ? 'male text-primary' : 'female text-danger'; ?> me-1"></i> <?= ucfirst(htmlspecialchars($row['tipe'], ENT_QUOTES)); ?></span>
                            </td>
                            ...
                            <td class="text-center"><span class="badge <?= $row['status'] == 'tersedia' ? 'bg-success' : 'bg-danger'; ?> rounded-pill"><?= ucfirst(htmlspecialchars($row['status'], ENT_QUOTES)); ?></span></td>
                            <td class="text-end pe-3">
                                <div class="btn-group">
                                    <button class="btn btn-sm btn-light border btn-edit" 
                                            data-id="<?= $row['id']; ?>" 
                                            data-nama="<?= htmlspecialchars($row['nama_kamar'], ENT_QUOTES); ?>"
                                            data-lokasi="<?= htmlspecialchars($row['lokasi'], ENT_QUOTES); ?>"
                                            data-harga="<?= htmlspecialchars($row['harga'], ENT_QUOTES); ?>"
                                            data-tipe="<?= htmlspecialchars($row['tipe'], ENT_QUOTES); ?>"
                                            data-status="<?= htmlspecialchars($row['status'], ENT_QUOTES); ?>"
                                            data-deskripsi="<?= htmlspecialchars($row['deskripsi'] ?? '', ENT_QUOTES); ?>"> <i class="fas fa-edit text-primary"></i>
                                    </button>
...
if (hargaTampil) {
    hargaTampil.addEventListener('input', function(e) {
        ...
        if (angkaMurni !== '') { this.value = new Intl.NumberFormat('id-ID').format(angkaMurni); } else { this.value = ''; }
    });
}
...
function setSampul(id, file) {
    fetch(`proses_kamar.php?set_sampul=1&id_kamar=${id}&file=${encodeURIComponent(file)}`).then(res => res.json()).then(res => {
